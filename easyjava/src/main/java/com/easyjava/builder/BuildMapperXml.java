    ...
    public static void execute(TableInfo tableInfo) {
        ...
        try {
            ...
            for (int i = 0, _len = fieldInfoList.size(); i < _len; i++) {
                bw.write(tableAlias + "." + fieldInfoList.get(i).getFieldName());
                if (i != _len - 1) {
                    bw.write(",");
                }
                if ((i + 1) % 5 == 0) {
            ...
            for (FieldInfo FieldInfo : fieldInfoList) {
                ...
                if (Constants.TYPE_DATE.equals(FieldInfo.getJavaType())) {
                    bw.write("\t\t\t\t <![CDATA[ and  " + tableAlias + "." + FieldInfo.getFieldName() + "=str_to_date(#{query." + FieldInfo.getPropertyName() + "}, '%Y-%m-%d') ]]>");
                } else {
                    bw.write("\t\t\t\t and  " + tableAlias + "." + FieldInfo.getFieldName() + " = #{query." + FieldInfo.getPropertyName() + "}");
                }
                bw.newLine();
                bw.write("\t\t\t</if>");
                bw.newLine();
            ...
            for (FieldInfo FieldInfo : fieldInfoList) {
                if (Constants.TYPE_STRING.equals(FieldInfo.getJavaType())) {
                    ...
                    bw.newLine();
                    bw.write("\t\t\t\t and  " + tableAlias + "." + FieldInfo.getFieldName() + " like concat('%', #{query." + FieldInfo.getPropertyName() + Constants
                ...
                if (Constants.TYPE_DATE.equals(FieldInfo.getJavaType())) {
                    ...
                    bw.newLine();
                    bw.write("\t\t\t\t <![CDATA[ and  " + tableAlias + "." + FieldInfo.getFieldName() + ">=str_to_date(#{query." + start + "}, '%Y-%m-%d') ]]>");
                    bw.newLine();
                    bw.write("\t\t\t</if>");
                    bw.newLine();
                    ...
                    bw.newLine();
                    bw.write("\t\t\t\t <![CDATA[ and  " + tableAlias + "." + FieldInfo.getFieldName() + "< date_sub(str_to_date(#{query." + end + "},'%Y-%m-%d')," +
                            "interval -1 day) ]]>");
                    bw.newLine();
                    bw.write("\t\t\t</if>");
                    bw.newLine();
            ...
            bw.write(" FROM " + tableInfo.getTableName() + " " + tableAlias);
            ...
            for (Map.Entry<String, List<FieldInfo>> entry : keyMap.entrySet()) {
                ...
                if (whereStr.length() > 0) {
                    ...
                    bw.newLine();
                    bw.write("\t\tselect <include refid=\"" + BASE_COLUMN_LIST + "\" /> from " + tableInfo.getTableName() + " " + tableAlias + " where " +
                            whereStr);
                    bw.newLine();
                    bw.write("\t</select>");
                    bw.newLine();
